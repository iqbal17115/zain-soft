                        ...
                        <div class="col-sm-12">
                            <div class="search-box mr-2 mb-2 d-inline-block">
                                <div class="position-relative">
                                    <h4 class="card-title design_title">Low Stock Report</h4>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="low_stock_report_table" class="table table-striped table-bordered nowrap"
...
<script>
    $(document).ready(function(){
		$('#low_stock_report_table').DataTable({
			processing: true,
			responsive: true,
			serverSide: true,
			ajax: {
				url: "{{ route('reports.low-stock-report-data') }}",
				type: 'GET',
				cache: false,
				data : function ( d ) {
			...
			{ title: 'Current Stock', data: 'stock', name: 'stock' },
			{ title: 'Low Stock Alert', data: 'low_stock_alert', class: "low_stock_alert", name: 'low_stock_alert' }
			],
			...
			footerCallback: function(row, data, start, end, display) {
				var api = this.api();
				$.each(['stock', 'low_stock_alert'], function( index, value ) {
					api.columns('.'+value, {
						page: 'all'
						}).every(function() {
						var sum = this
		...
		$(document).on('change','.updateTable',function () {
			$('#low_stock_report_table').DataTable().draw(true);
		});
	});
</script>
@endsection
