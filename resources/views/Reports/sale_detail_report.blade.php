                        ...
                        <div class="col-sm-12">
                            <div class="search-box mr-2 mb-2 d-inline-block">
                                <div class="position-relative">
                                    <h4 class="card-title design_title">Sale Details Report</h4>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="sale_detail_report_table" class="table table-striped table-bordered nowrap"
...
<script>
    $(document).ready(function(){
		...
		$('#sale_detail_report_table').DataTable({
			processing: true,
			responsive: true,
			serverSide: true,
			ajax: {
				url: "{{ route('reports.sale-detail-report-data') }}",
				type: 'GET',
				cache: false,
				data : function ( d ) {
			...
			{ title: 'Sale Code', data: 'invoice_code', name: 'invoice_code' },
			{ title: 'Customer Name', data: 'contact_id', name: 'contact_id' },
			{ title: 'Item Name', data: 'item_id', name: 'item_id' },
			{ title: 'Qty', data: 'quantity', class: "quantity", name: 'quantity' },
			{ title: 'Sale Price(AED)', data: 'sale_price', class: "sale_price", name: 'sale_price' },
			{ title: 'Sub Total', data: 'subtotal', class: "subtotal", name: 'subtotal' },
			...
			footerCallback: function(row, data, start, end, display) {
				var api = this.api();
				$.each(['quantity', 'sale_price', 'subtotal'], function( index, value ) {
					api.columns('.'+value, {
						page: 'all'
						}).every(function() {
						var sum = this
		...
		$(document).on('change','.updateTable',function () {
			$('#sale_detail_report_table').DataTable().draw(true);
		});
	});
</script>
@endsection
