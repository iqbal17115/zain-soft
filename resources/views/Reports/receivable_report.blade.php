            ...
            <div class="card">
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="receivable_report_table" class="table table-striped table-bordered nowrap"
...
<script>
    $(document).ready(function(){
		...
		$('#receivable_report_table').DataTable({
			processing: true,
			responsive: true,
			serverSide: true,
			ajax: {
				url: "{{ route('reports.receivable-report-data') }}",
				type: 'GET',
				cache: false,
				data : function ( d ) {
			...
			footerCallback: function(row, data, start, end, display) {
				var api = this.api();
				$.each(['opening_balance', 'credit', 'debit', 'closing_balance'], function( index, value ) {
		...
		$(document).on('change','.updateTable',function () {
			$('#receivable_report_table').DataTable().draw(true);
		});
	});
</script>
@endsection
