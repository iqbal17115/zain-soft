...
@endpush
<div>
    <x-slot name="title">Quotation LIST</x-slot>
    <x-slot name="header">Quotation LIST</x-slot>
    <div class="row mb-4">
                        ...
                        <div class="col-lg-4 col-md-4 col-sm-4 col-4">
                            <div class="search-box mr-2 mb-2 d-inline-block">
                                <div class="position-relative">
                                    <h4 class="card-title mb-3">Quotation List</h4>
                                </div>
                            </div>
                        </div>
                        <div class="col-lg-8 col-md-8 col-sm-8 col-8">
                            <div class="text-sm-right">
                                <a href="{{ route('inventory.quotation') }}"> <button type="button"
                                        style="float: right;"
                                        class="btn btn-success btn-rounded waves-effect waves-light mb-2 mr-2"
                                        data-toggle="modal" data-target=".bd-example-modal-lg"><i
                                            class="mdi mdi-plus mr-1"></i>New Quotation</button></a>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <div wire:ignore class="table-responsive">
        <table class="display table table-striped table-bordered" id="QuotationListTable" style="width:100%"></table>
    </div>
                ...
                <div class="modal-body">
                    ...
                    <div>
                        ...
                        <div id="invoice_page">
                                    ...
                                    <div style="float:left;margin-top:0px;width: 74%;">
                                        <div class="row">
                                            <div style="float:left;width: 25%;">
                                                <span><img height=60px; width=100px;
                                            ...
                                            <div style="float:left; width: 75%; position: relative;">
                                                <h1
                                                    ...
                                                    {{Auth::user()->Company->name}}@else
                                                    {{ $profile_setting->address }}
                                                    @endif
                                                </h1>
                                                <p style="margin:0px;padding:0px;font-size:13px;font-weight:bold">
                                                    Address: @if(isset(Auth::user()->Company->address))
                                                    {{Auth::user()->Company->address}}@else
                                                    {{ $profile_setting->address }}
                                                    @endif <br />
                                                    Postal Code: @if(isset(Auth::user()->Company->postal_code))
                                                    {{Auth::user()->Company->postal_code}}@else
                                                    {{ $profile_setting->address }}
                                                    @endif <br />

                                                    TRN NO: @if(isset(Auth::user()->Company->trn))
                                                    {{Auth::user()->Company->trn}}@else
                                                    {{ $profile_setting->address }}
                                                    @endif <br />

                                                    Mobile: @if(isset(Auth::user()->Company->mobile))
                                                    {{Auth::user()->Company->mobile}}@else
                                                    {{ $profile_setting->address }}
                                                    @endif <br />

                                                    Telephone: @if(isset(Auth::user()->Company->telephone))
                                                    {{Auth::user()->Company->telephone}}@else
                                                    {{ $profile_setting->address }}
                                                    @endif <br />

                                                    Email: @if(isset(Auth::user()->Company->telephone))
                                                    {{Auth::user()->Company->telephone}}@else
                                                    {{ $profile_setting->address }}
                                                    @endif <br />

                                                    Website: @if(isset(Auth::user()->Company->web_address))
                                                    {{Auth::user()->Company->web_address}}@else
                                                    {{ $profile_setting->address }}
                                                    @endif <br />
                                                </p>
                                            </div>
                                        ...
                                        <p
                                            ...
                                            Bill To</p>
                                        <h6><strong>Supplier Name @php echo str_repeat('&nbsp;', 7); @endphp:
                                            </strong><strong>@if ($invoice) {{ $invoice->Contact->name }}@endif</strong>
                                        </h6>
                                        <h6><strong>Contact Person @php echo str_repeat('&nbsp;',6);
                                                @endphp:</strong><strong>@if ($invoice) {{
                                                $invoice->Contact->business_name }}@endif</strong> </h6>
                                        <h6><strong>Telephone @php echo str_repeat('&nbsp;', 14);
                                                @endphp:</strong><strong>@if ($invoice) {{ $invoice->Contact->telephone
                                                }}@endif</strong></h6>
                                        <h6><strong>Mobile @php echo str_repeat('&nbsp;', 20);
                                                @endphp:</strong><strong>@if ($invoice) {{ $invoice->Contact->mobile }}
                                                @endif</strong></h6>
                                        <h6><strong>TRN @php echo str_repeat('&nbsp;', 24); @endphp:</strong>
                                            <strong>@if ($invoice) {{ $invoice->Contact->trn_no }} @endif</strong></h6>
                                        <h6><strong>Email @php echo str_repeat('&nbsp;', 22); @endphp:</strong>
                                            <strong>@if ($invoice) {{ $invoice->Contact->email }} @endif</strong></h6>
                                        <h6><strong>Address @php echo str_repeat('&nbsp;', 18); @endphp:</strong>
                                            <strong>@if ($invoice) {{ $invoice->Contact->address }} @endif</strong>
                                        </h6>
                                        <h6><strong>Place of Supplier @php echo str_repeat('&nbsp;', 3);
                                                @endphp:</strong> <strong>{{ $invoice->Contact->division }}{{
                                                $invoice->Contact->country }}</strong></h6>
                                        <!--h6>Delivery By &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;:&nbsp;&nbsp;&nbsp;&nbsp; Nazmul</h6-->
                                    </div>
                                    <div style="float:right;width: 25%; position: relative; bottom: 26px;">
                                        {{-- <h3
                                            style="color:red;font-weight:bold;margin-left:0px;margin-top:0px;font-size:20px;">
                                            {{$invoice->payment_status}}</h3> --}}
                                        <h6 style="margin-top:50px;font-weight:bold;font-size:20px;color:lightskyblue">
                                            Quotation</h6>
                                        <h6><strong>Quotation No @php echo str_repeat('&nbsp;', 1);
                                                @endphp:</strong><strong> {{ $invoice->code }}</strong></h6>
                                        <h6><strong>Date @php echo str_repeat('&nbsp;', 11); @endphp:</strong>
                                            <strong>{{ date('Y/m/d') }}</strong></h6>
                                    </div>
                                </div>
                                <br />
                            </div>
                            ...
                            <div class="col-md-12 heading" style="position: relative; right: 16px;">
                                <table>
                                    <tr class="inv_item_row">
                                        <td>Amount Chargeable (In Words):<br> @if (isset($numberToWordTotal)){{
...
@push('scripts')
      ...
      <script>
        function generatePDF(id) {
            ...
            var opt = {
                margin: 1,
                filename: 'Quotation-invoice.pdf',
                image: {
                    type: 'jpeg',
                    quality: 0.98
                },
    ...
    <script>
        function callInvoice(id) {
            ...
            function callDelete(id) {
                @this.call('QuotationDelete', id);
            }

            $(document).ready(function() {

                var datatable = $('#QuotationListTable').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: "{{ route('data.quotation-list-table') }}",
                    columns: [{
                            title: 'SL',
                            data: 'id'
                        },
