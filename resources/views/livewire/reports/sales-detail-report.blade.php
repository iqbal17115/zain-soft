...
<div>
    <x-slot name="title">SALES DETAILED REPORT</x-slot>
    <x-slot name="header">SALES DETAILED REPORT</x-slot>
    <div class="row mb-4">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                        ...
                        <div class="col-sm-12">
                            <div class="search-box mr-2  d-inline-block">
                                <div class="position-relative">
                                    <h4 class="card-title" id="header-text-design">Sales Detailed Report</h4>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table id="datatable-buttons" class="table table-striped table-bordered nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    ...
                    <tbody>
                        @php $i=0; $sale_price=0; $sale_subtotal=0; @endphp
                        @foreach ($this->dateFilter($saleDetails) as $saleDetail)
                        <tr>
                            <td><a href="javascript: void(0);" class="text-body font-weight-bold">{{ ++$i }}</a></td>
                            <td>{{ $saleDetail->date }}</td>
                            {{-- <td>{{ $saleDetail->Contact->name }}</td> --}}
                            <td>
                                @if($saleDetail->Contact) {{ $saleDetail->Contact->name }} @endif
                            </td>
                            <td>
                                {{ $saleDetail->Item->name }}
                            </td>
                            <td>
                                {{ $saleDetail->quantity }} {{$saleDetail->Item->Unit->name}}
                            </td>
                            <td>
                                {{ $saleDetail->sale_price }}
                                 @php $sale_price += $saleDetail->sale_price @endphp
                            </td>
                            <td>
                                {{ $saleDetail->subtotal }}
                                @php $sale_subtotal += $saleDetail->subtotal @endphp
                            </td>
                            <td>
                                @if($saleDetail->Branch)
                                {{ $saleDetail->Branch->name }}
                                @endif
                            </td>
                        </tr>
                        @endforeach
