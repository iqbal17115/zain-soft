...
<div>
    <x-slot name="title">Payable REPORT</x-slot>
    <x-slot name="header">Payable REPORT</x-slot>
    <div class="row mb-4">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                        ...
                        <div class="col-sm-12">
                            <div class="search-box mr-2  d-inline-block">
                                <div class="position-relative">
                                    <h4 class="card-title" id="header-text-design">Payable Report</h4>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        ...
                        <div wire:ignore class="col-md-4">
                            <div class="form-group">
                                <label for="basicpill-firstname-input">Search Supplier</label>
                ...
                <table id="example" class="table table-striped table-bordered nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{$contact->name}}</td>
                            <td>
                                {{$this->getPayable(['contact_id'=>$contact->id,'start_date'=>$start_date,'end_date'=>$end_date])->opening_balance}}
                            </td>
                            <td>
                                {{$this->getPayable(['contact_id'=>$contact->id,'start_date'=>$start_date,'end_date'=>$end_date])->total_debit}}

                                @php
                                  $debit += $this->getPayable(['contact_id'=>$contact->id,'start_date'=>$start_date,'end_date'=>$end_date])->total_debit;
                                @endphp
                            </td>
                            <td>
                                {{$this->getPayable(['contact_id'=>$contact->id,'start_date'=>$start_date,'end_date'=>$end_date])->total_credit}}


                                @php
                                  $credit += $this->getPayable(['contact_id'=>$contact->id,'start_date'=>$start_date,'end_date'=>$end_date])->total_credit;
                                @endphp
                            </td>
                            <td>
                                {{$this->getPayable(['contact_id'=>$contact->id])->current_balance}}
                                {{-- {{($crOpening-$drOpening )+($cr-$dr)}} --}}
                                @php
                                  $closing += $this->getPayable(['contact_id'=>$contact->id])->current_balance;
                                @endphp
                            </td>
                        </tr>
                        @endforeach
